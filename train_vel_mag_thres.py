...
class Dataset:
  ...
  def _convert_to_torch(self):
    for idx in range(len(self.input_data)):
      self.input_data[idx] = torch.from_numpy(self.input_data[idx]).to(torch.float)
...
net = Net()
criterion =  nn.CrossEntropyLoss() #nn.BCELoss() #nn.BCEWithLogitsLoss() #nn.L1Loss()#
optimizer =optim.Adam(net.parameters(),lr=5e-4) # optim.SGD(net.parameters(), lr=0.001, momentum=0.9)
...
for epoch in range(no_epochs):  # loop over the dataset multiple times
    ...
    writer.add_scalars("Loss/epoch", {'valid':avg_v_loss, 'train':avg_t_loss}, epoch)
...
PATH = './threshold.pth'
torch.save(net.state_dict(), PATH)s